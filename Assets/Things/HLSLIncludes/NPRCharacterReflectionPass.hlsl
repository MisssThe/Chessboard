...
float4 _PlanePositionWS;
float _CharacterHeightBlend;
float _CharacterHeightBlendSoftAlpha;

void HeightBlend(inout float alpha, float3 positionWS)
{
	alpha = positionWS.y - _PlanePositionWS.y;
	alpha = smoothstep(0, _CharacterHeightBlend, alpha);
	alpha = smoothstep(0.5f - _CharacterHeightBlendSoftAlpha * 0.5f, 0.5f + _CharacterHeightBlendSoftAlpha * 0.5f, alpha);
...
half LinearStep(half minValue, half maxValue, half In)
{
 ...
 half4 NPRShading(NPRBaseData nprbaseData, InputData inputData)
 {
	//===================基础数据准备===================
	...
	finalCol += emissionCol;
	 ...
	 return half4(finalCol,nprbaseData.alpha);
 }

half4 ReflectionPassFragment (Varyings input) : SV_Target
{
	...
	half4 color = NPRShading(nprbaseData, inputData);
	
	// Height Blend
	HeightBlend(color.a, input.positionWS);
